...
import { ArrowLeft, ListTodo, Paperclip, Pencil } from "lucide-react";
...
type ReportAttachmentRow = ReportAttachmentsRow;
...
export default async function InternReportDetailPage({
  ...
  const canEdit = report.status === "draft" || report.status === "needs_revision";
  ...
  return (
    <div className="flex flex-col gap-6">
      <PageHeader
        ...
        actions={
          <div className="flex flex-wrap gap-2">
            {canEdit ? (
              <Link
                href={`/intern/reports/${report.id}/edit`}
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        ...
        <div className="flex flex-col gap-1 rounded-lg border p-4 text-sm">
          <span className="text-muted-foreground">Đợt thực tập</span>
          <span className="font-medium">
            {report.interns
      ...
      {report.review_comment ? (
        <div className="rounded-lg border bg-muted/40 px-4 py-3 text-sm">
          <span className="font-medium">Nhận xét của mentor: </span>
          {report.review_comment}
        </div>
      ) : null}
      ...
      <ReportDetailActions reportId={report.id} status={report.status} mode="mine" />
